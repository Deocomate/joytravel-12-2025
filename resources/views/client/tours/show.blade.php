        ...
        <div class="container mx-auto px-4 py-6 md:py-10">
            ...
            <nav class="mb-4 text-sm" aria-label="Breadcrumb">
                <ol class="flex items-center gap-2 text-gray-500 flex-wrap">
                    <li><a href="{{ route('client.home') }}" class="hover:text-[var(--color-primary)] transition-colors">Trang
                            chủ</a></li>
                    <li><i class="fa-solid fa-chevron-right text-xs"></i></li>
                    <li><a href="{{ route('client.tours') }}" class="hover:text-[var(--color-primary)] transition-colors">Tour
                            du lịch</a></li>
                    <li><i class="fa-solid fa-chevron-right text-xs"></i></li>
                    <li class="text-gray-800 font-medium truncate max-w-[200px]">{{ $tour->name ?? 'Chi tiết' }}</li>
                </ol>
            </nav>

            {{-- Tour Header --}}
            <div class="mb-6">
                <h1 class="text-2xl md:text-3xl font-extrabold text-gray-800 leading-tight">
                    {{ $tour->name ?? 'Tên tour đang cập nhật' }}</h1>
                <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-gray-500 mt-3">
                    <span class="bg-gray-100 px-3 py-1 rounded-full">
                        <i class="fa-solid fa-hashtag text-[var(--color-primary)] mr-1"></i>
                        {{ $tour->tour_code ?? 'N/A' }}
                    </span>
                    @if ($tour->duration)
                    ...
                    @if ($tour->categories && $tour->categories->isNotEmpty())
                        <span class="flex items-center gap-1">
                            <i class="fa-solid fa-tag text-[var(--color-primary)]"></i>
