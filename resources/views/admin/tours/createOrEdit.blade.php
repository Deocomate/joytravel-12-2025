...
@section('content')
    <form action="{{ $isEdit ? route('admin.tours.update', $tour) : route('admin.tours.store') }}" method="POST" novalidate>
            ...
            <div class="col-md-8">
                <div class="card card-primary">
                    ...
                    <div class="card-body">
                        ...
                        <x-admin.inputs.text label="Tên tour" name="name" :value="old('name', $tour->name ?? '')" required/>
                        <x-admin.inputs.text-area label="Mô tả ngắn" name="short_description" :value="old('short_description', $tour->short_description ?? '')"/>
                        <x-admin.inputs.editor label="Mô tả chi tiết tour" name="tour_description" :value="old('tour_description', $tour->tour_description ?? '')"/>

                        {{-- Lịch trình tour --}}
                        <x-admin.inputs.tour-schedule-array label="Lịch trình tour" name="tour_schedule" :value="old('tour_schedule', $tour->tour_schedule ?? [])"/>

                        <x-admin.inputs.editor label="Ghi chú dịch vụ" name="services_note" :value="old('services_note', $tour->services_note ?? '')"/>
                        <x-admin.inputs.editor label="Ghi chú thêm" name="note" :value="old('note', $tour->note ?? '')"/>
                        <x-admin.inputs.editor label="Đặc điểm nổi bật" name="characteristic" :value="old('characteristic', $tour->characteristic ?? '')"/>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="card card-secondary">
                    ...
                    <div class="card-body">
                        ...
                        <hr>
                        <x-admin.inputs.text label="Thời gian" name="duration" :value="old('duration', $tour->duration ?? '')"/>
                        <x-admin.inputs.text label="Điểm khởi hành" name="departure_point" :value="old('departure_point', $tour->departure_point ?? '')"/>
                        <x-admin.inputs.text label="Phương tiện" name="transport_mode" :value="old('transport_mode', $tour->transport_mode ?? '')"/>
                        <x-admin.inputs.number label="Số chỗ còn lại" name="remaining_slots" :value="old('remaining_slots', $tour->remaining_slots ?? 0)"/>
                        <x-admin.inputs.number label="Độ ưu tiên" name="priority" :value="old('priority', $tour->priority ?? 0)"/>
                    </div>
                </div>
                <div class="card card-warning">
                    <div class="card-header"><h3 class="card-title">Giá vé</h3></div>
                    <div class="card-body">
                        <x-admin.inputs.number label="Giá người lớn (> 11 tuổi)" name="price_adult" :value="old('price_adult', $tour->price_adult ?? 0)"/>
                        <x-admin.inputs.number label="Giá trẻ em (5 - 11 tuổi)" name="price_child" :value="old('price_child', $tour->price_child ?? 0)"/>
                        <x-admin.inputs.number label="Giá trẻ nhỏ (2 - 5 tuổi)" name="price_toddler" :value="old('price_toddler', $tour->price_toddler ?? 0)"/>
                        <x-admin.inputs.number label="Giá em bé (< 2 tuổi)" name="price_infant" :value="old('price_infant', $tour->price_infant ?? 0)"/>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card card-info">
            ...
            <div class="card-body">
                <x-admin.inputs.image-link-array label="" name="images" :value="old('images', $tour->images ?? [])"/>
            </div>
        </div>
