...
@section('content')
    <form action="{{ route('admin.contacts.update') }}" method="POST">
        ...
        <div class="card card-primary">
            ...
            <div class="card-body">
                ...
                <x-admin.inputs.text label="Tên công ty" name="company_name" :value="old('company_name', $contact->company_name ?? '')"/>
                <div class="row">
                    <div class="col-md-6">
                        <x-admin.inputs.email label="Email chính" name="email" :value="old('email', $contact->email ?? '')"/>
                    </div>
                    <div class="col-md-6">
                        <x-admin.inputs.text label="Số điện thoại chính" name="phone" :value="old('phone', $contact->phone ?? '')"/>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <x-admin.inputs.text label="Facebook" name="facebook" :value="old('facebook', $contact->facebook ?? '')"/>
                    </div>
                    <div class="col-md-6">
                        <x-admin.inputs.text label="Zalo" name="zalo" :value="old('zalo', $contact->zalo ?? '')"/>
                    </div>
                </div>
                <x-admin.inputs.text label="Giờ làm việc" name="working_hours" :value="old('working_hours', $contact->working_hours ?? '')"/>
            </div>
        </div>
        ...
        <div class="card card-info">
            ...
            <div class="card-body" id="branches-container">
                ...
                @foreach($branches as $index => $branch)
                    @include('admin.contact.partials.branch-item', ['index' => $index, 'branch' => (object)$branch, 'mainBranchIndex' => $mainBranchIndex])
                @endforeach
            </div>
        </div>
    ...
    <template id="branch-template">
        @include('admin.contact.partials.branch-item', ['index' => '__INDEX__', 'branch' => null, 'mainBranchIndex' => -1])
    </template>
@endsection
