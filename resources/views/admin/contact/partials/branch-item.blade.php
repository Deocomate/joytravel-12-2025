<div class="card card-outline card-secondary branch-item">
    ...
    <div class="card-body">
        ...
        <x-admin.inputs.text label="Tên chi nhánh" name="branches[{{ $index }}][branch_name]"
                       :value="$branch->branch_name ?? ''" required/>
        <x-admin.inputs.text label="Địa chỉ" name="branches[{{ $index }}][address]" :value="$branch->address ?? ''"/>
        <div class="row">
            <div class="col-md-6">
                <x-admin.inputs.text label="Số điện thoại" name="branches[{{ $index }}][phone]"
                               :value="$branch->phone ?? ''"/>
            </div>
            <div class="col-md-6">
                <x-admin.inputs.email label="Email" name="branches[{{ $index }}][email]" :value="$branch->email ?? ''"/>
            </div>
        </div>
        <x-admin.inputs.text label="Giờ làm việc" name="branches[{{ $index }}][working_hours]"
