(function (global) {
    ...
    (function () {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            mobileMenu?.querySelectorAll('a:not([data-modal-target]):not([data-modal-switch])').forEach(link => {
                link.addEventListener('click', closeMobileMenu);
            });

            document.addEventListener('click', (event) => {
                if (guestDropdown && guestDropdown.classList.contains('active') && guestAccountButton && !guestAccountButton.contains(event.target) && !guestDropdown.contains(event.target)) {
                    guestDropdown.classList.remove('active');
                }

                if (accountDropdown && accountDropdown.classList.contains('active') && accountButton && !accountButton.contains(event.target) && !accountDropdown.contains(event.target)) {
